...
export function QuestsManager({ onRefresh }: QuestsManagerProps) {
    ...
    const [rewardDecorationId, setRewardDecorationId] = useState<number | undefined>();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            ...
            reward_badge_id: (rewardType === 'badge' || rewardType === 'both') ? (rewardBadgeId || null) : null,
            reward_decoration_id: (rewardType === 'decoration' || rewardType === 'both') ? (rewardDecorationId || null) : null,
            is_active: isActive,
    ...
    const selectedDecoration = decorations.find(d => d.id === rewardDecorationId);
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            {questType === 'genre_read' && (
                                <div>
                                    <label className="block text-sm text-gray-400 mb-1">Target Genre</label>
                                    <input
                                        type="text"
                                        value={targetGenre}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {(rewardType === 'badge' || rewardType === 'both') && (
                                <div>
                                    <label className="block text-sm text-gray-400 mb-1">Reward Badge</label>
                                    <div className="flex gap-2 items-center">
                                        {selectedBadge && (
                                            <div className="w-8 h-8 rounded-lg bg-[#2a2a2a] border border-white/10 p-1 flex-shrink-0">
                                                <img src={resolveUrl(selectedBadge.icon_url)} alt={selectedBadge.name} className="w-full h-full object-contain" />
                                            </div>
                                        )}
                                        <select
                                            value={rewardBadgeId || ''}
                                            onChange={e => setRewardBadgeId(e.target.value ? Number(e.target.value) : undefined)}
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {badges.map(b => (
                                                <button
                                                    ...
                                                    title={b.name}
                                                >
                                                    <img src={resolveUrl(b.icon_url)} alt={b.name} className="w-full h-full object-contain" />
                                                </button>
                                            ))}
                                        </div>
                                    )}
                            ...
                            {(rewardType === 'decoration' || rewardType === 'both') && (
                                <div>
                                    <label className="block text-sm text-gray-400 mb-1">Reward Decoration</label>
                                    <div className="flex gap-2 items-center">
                                        {selectedDecoration && !selectedDecoration.image_url.startsWith('css:') && (
                                            <div className="w-8 h-8 rounded-lg bg-[#2a2a2a] border border-white/10 p-1 flex-shrink-0">
                                                <img src={resolveUrl(selectedDecoration.image_url)} alt={selectedDecoration.name} className="w-full h-full object-contain" />
                                            </div>
                                        )}
                                        <select
                                            value={rewardDecorationId || ''}
                                            onChange={e => setRewardDecorationId(e.target.value ? Number(e.target.value) : undefined)}
                                            className="flex-1 bg-[#1a1a1a] border border-white/10 rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-primary/50"
                                        >
                                    ...
                                    {/* Decoration grid for quick selection */}
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {decorations.map(d => (
                                                <button
                                                    ...
                                                    className={`w-10 h-10 rounded-lg border p-1 transition-all ${rewardDecorationId === d.id ? 'border-purple-400 bg-purple-500/10 scale-110' : 'border-white/10 bg-[#2a2a2a] hover:border-white/30'}`}
                                                    title={d.name}
                                                >
                                                    {d.image_url.startsWith('css:') ? (
                                                        <span className="text-[8px] text-purple-400 font-mono leading-tight block">{d.image_url.split(':')[1]}</span>
                                                    ) : (
                                                        <img src={resolveUrl(d.image_url)} alt={d.name} className="w-full h-full object-contain" />
                                                    )}
                                                </button>
                                            ))}
                                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Starts At (optional)</label>
                                <input
                                    type="datetime-local"
                                    value={startsAt}
                                ...
                                <tr key={quest.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    <td className="px-4 py-3">
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-xs truncate max-w-xs">{quest.description}</p>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3 text-center text-white">{quest.target_value}</td>
                                    <td className="px-4 py-3">
