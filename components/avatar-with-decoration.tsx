...
export const AvatarWithDecoration: React.FC<AvatarWithDecorationProps> = ({
  ...
  if (decorationUrl?.startsWith("css:")) {
    ...
    switch (type) {
      case "pop-art":
        content = (
          <PopArtAvatar className={config.avatar}>{avatarElement}</PopArtAvatar>
        );
        break;
      case "manga-speed":
        content = (
          <MangaSpeedAvatar className={config.avatar}>
            {avatarElement}
          </MangaSpeedAvatar>
        );
        break;
      case "cyberpunk":
        content = (
          <CyberpunkAvatar className={config.avatar}>
            {avatarElement}
          </CyberpunkAvatar>
        );
        break;
      case "webtoon":
        content = (
          <WebtoonPanelsAvatar className={config.avatar}>
            {avatarElement}
          </WebtoonPanelsAvatar>
        );
        break;
      case "halftone":
        content = (
          <HalftoneNoirAvatar className={config.avatar}>
            {avatarElement}
          </HalftoneNoirAvatar>
        );
        break;
  ...
  return (
    <div className={cn("relative flex items-center justify-center", className)}>
      ...
      {decorationUrl && !decorationUrl.startsWith("css:") && (
        ...
        >
          <img
            src={resolveDecorationUrl(decorationUrl)}
      ...
      {badges && badges.length > 0 && (
        <div className="absolute -bottom-1 -right-1 z-20 flex -space-x-1">
          {badges.slice(0, 3).map((badge, idx) => (
            ...
            >
              <img
                src={resolveBadgeUrl(badge.icon_url)}
                alt={badge.name}
