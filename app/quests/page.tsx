...
export default function QuestsPage() {
  ...
  const hasDecorationOnly = (quest: Quest) =>
    !quest.badge_icon_url && !!quest.decoration_image_url;
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        >
          ...
          <p className="text-[var(--muted-foreground)] text-lg max-w-2xl mx-auto">
            Selesaikan quest untuk mendapatkan badge eksklusif dan avatar
            decoration!
          </p>
          {!user && (
        ...
        {loading && (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-[var(--card)]/50 rounded-2xl p-6 animate-pulse border border-[var(--border)]"
              >
                  ...
                  >
                    ...
                    <div className="p-6">
                      <div className="flex items-start gap-5">
                        {/* Badge / Decoration Icon */}
                        <div className="relative flex-shrink-0 w-20 h-20 flex items-center justify-center">
                          {badgeUrl ? (
                            /* Badge reward — circular image */
                            <div className="relative w-20 h-20 rounded-full overflow-hidden bg-[var(--background)]">
                              <Image
                                src={badgeUrl}
                                alt={quest.title}
                                fill
                            ...
                            /* Decoration-only reward — show AvatarWithDecoration preview */
                            <div
                              className={`transition-all duration-300 ${quest.is_completed ? "" : "opacity-60 grayscale"}`}
                            >
                            ...
                            /* Fallback — trophy gradient */
                            <div
                              className={`w-20 h-20 rounded-full bg-gradient-to-br ${getQuestTypeColor(quest.quest_type)} flex items-center justify-center`}
                            >
                              <Trophy
                                className={`w-8 h-8 ${quest.is_completed ? "text-amber-400" : "text-white/50"}`}
                              />
                            </div>
                          )}
                          {quest.is_completed && (
                            <div className="absolute -top-1 -right-1 bg-emerald-500 rounded-full p-0.5 z-20">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span
                              className={`text-xs font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-gradient-to-r ${getQuestTypeColor(quest.quest_type)} text-white`}
                            >
                          ...
                          <div className="flex items-center gap-2 mb-3 flex-wrap">
                            ...
                            {quest.decoration_name && (
                              <span className="inline-flex items-center gap-1.5 text-xs bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2.5 py-1 rounded-full">
                                {/* Mini decoration preview */}
                                <span className="inline-flex items-center justify-center w-4 h-4 scale-[0.85]">
                                  <AvatarWithDecoration
                                    src="https://picsum.photos/seed/comic/150/150"
                                    fallback="U"
                                    decorationUrl={quest.decoration_image_url}
                          ...
                          <div className="space-y-1.5">
                            <div className="flex justify-between text-xs">
                              ...
                              <span
                                className={`font-mono font-bold ${quest.is_completed ? "text-emerald-400" : "text-[var(--foreground)]"}`}
                              >
