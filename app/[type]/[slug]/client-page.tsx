...
export default function MangaDetailPage({ initialData }: MangaDetailPageProps) {
  ...
  const [detail, setDetail] = useState<AggregatedMangaDetail | null>(
    initialData || null,
  );
    ...
    <main className="min-h-screen bg-background text-foreground pb-24 relative overflow-x-hidden">
      ...
      <div className="relative w-full h-[300px] md:h-[400px]">
        ...
        <div className="container mx-auto px-4 h-full flex flex-col justify-end pb-8 relative z-10">
          <Link
            href="/"
            className="absolute top-24 left-4 md:left-0 flex items-center gap-2 text-white/80 hover:text-white transition-colors bg-black/20 backdrop-blur-sm px-3 py-1 rounded-full"
          >
          ...
          >
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                ...
                <span className="flex items-center gap-1">
                  <User className="w-4 h-4 text-blue-400" /> {detail.author}
                </span>
                <span className="flex items-center gap-1">
                  <Star className="w-4 h-4 text-yellow-500" />{" "}
                  {selectedSource.rating || 0}
                </span>
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold ${detail.status === "Ongoing" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                >
                  {detail.status}
                </span>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {detail.genres.map((genre) => (
                    <Link
                      ...
                      className="px-3 py-1 rounded-full bg-secondary text-xs font-medium text-secondary-foreground hover:bg-primary hover:text-white transition-colors"
                    >
                      {genre}
                    </Link>
                  ))}
          ...
          <div className="grid gap-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {selectedSource.chapters.map((chapter, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`font-medium transition-colors line-clamp-1 flex-1 min-w-0 pr-2 ${
                      readStatus?.has(chapter.id, chapter.title)
